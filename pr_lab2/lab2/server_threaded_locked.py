...
def list_dir_with_counters(fs_path, rel_url):
    ...
    for name in sorted(os.listdir(fs_path)):
        ...
        count = snapshot.get(os.path.join(rel_url, name).rstrip("/"), 0)
    ...
    <p><strong>Thread-safe counters.</strong></p>
    </body></html>"""
...
def update_counter_safe(rel_url):
    with counts_lock:
        request_counts[rel_url] += 1
...
def handle(conn, addr):
    try:
        ...
        if os.path.isdir(fs_path) and not target.endswith("/"):
            ...
            return
        update_counter_safe(rel.rstrip("/") if rel else "/")
...
def main():
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        s.bind((HOST, PORT))
        s.listen(128)
        print(f"[lab2-safe] Serving on http://{HOST}:{PORT}")
        while True:
            conn, addr = s.accept()
